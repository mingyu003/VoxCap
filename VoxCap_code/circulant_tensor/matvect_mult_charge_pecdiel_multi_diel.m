function [JOut_full]=matvect_mult_charge_pecdiel_multi_diel(ch_coef_dum, fN_ch_all,inds_glob,locs_diel_pos,ids_panel_diel,ids_panels,L,M,N,num_diel,ss,fl_Tucker_decomp,ids,precond,seg,ord,fl_precond)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (num_diel > 0)
    ch_coefs_xx = ch_coef_dum(inds_glob(1,1):inds_glob(num_diel+1,2)); % [rhoa_x_c;rhoa_x_d]
    ch_coefs_yy = ch_coef_dum(inds_glob(num_diel+2,1):inds_glob(2*(num_diel+1),2)); % [rhoa_y_c;rhoa_y_d]
    ch_coefs_zz = ch_coef_dum(inds_glob(2*(num_diel+1)+1,1):inds_glob(3*(num_diel+1),2)); % [rhoa_z_c;rhoa_z_d]
    
    coe_xx=cell(num_diel,1);coe_yy=cell(num_diel,1);coe_zz=cell(num_diel,1);
    
    for ii=1:num_diel
        coe_xx{ii}=ss(ii)*ch_coef_dum(inds_glob(1+ii,1):inds_glob(1+ii,2)); % coefficient for self-term extraction
        coe_yy{ii}=ss(ii)*ch_coef_dum(inds_glob(num_diel+2+ii,1):inds_glob(num_diel+2+ii,2));
        coe_zz{ii}=ss(ii)*ch_coef_dum(inds_glob(2*(num_diel+1)+1+ii,1):inds_glob(2*(num_diel+1)+1+ii,2));
    end
...
temp=ids_panels{1}{1};
for ii=1:num_diel
    temp=[temp;ids_panels{1+ii}{1}];
end
JIn_x(temp) = ch_coefs_xx;
JIn_y = zeros(L, M+1, N);
temp=ids_panels{1}{2};
for ii=1:num_diel
    temp=[temp;ids_panels{1+ii}{2}];
end
JIn_y(temp) = ch_coefs_yy;
JIn_z = zeros(L, M, N+1);
temp=ids_panels{1}{3};
for ii=1:num_diel
    temp=[temp;ids_panels{1+ii}{3}];
end
...
% if (num_diel > 0)
    JOut_full(inds_glob(num_diel+2,1):inds_glob(num_diel+2,2)) = Jout1_y(ids_panels{1}{2});
% else
%     JOut_full(inds_glob(2,1):inds_glob(2,2)) = Jout1_y(ids_panels{1}{2});
% end
Jout1_z = Jout1_z(1:L,1:M,1:N+1);
% if (num_diel > 0)
    JOut_full(inds_glob(2*(num_diel+1)+1,1):inds_glob(2*(num_diel+1)+1,2)) = Jout1_z(ids_panels{1}{3});
% else
...
if (num_diel > 0)
    ...
    for ii=1:num_diel
        JOut_full(inds_glob(1+ii,1)+locs_diel_pos{ii}{1}-1) = Jout1_x(ids_panel_diel{ii}{1});
        JOut_full(inds_glob(1+ii,1)+locs_diel_pos{ii}{4}-1) = -Jout1_x(ids_panel_diel{ii}{4});
        JOut_full(inds_glob(1+ii,1):inds_glob(1+ii,2))=coe_xx{ii}+JOut_full(inds_glob(1+ii,1):inds_glob(1+ii,2));
    end
    %
    %     JOut_full(inds_glob(3,1)+locs_diel_pos{2}{1}-1) = Jout1_x(ids_panel_diel{2}{1});
    %     JOut_full(inds_glob(3,1)+locs_diel_pos{2}{4}-1) = -Jout1_x(ids_panel_diel{2}{4});
    %     JOut_full(inds_glob(3,1):inds_glob(3,2))=coe_xx2+JOut_full(inds_glob(3,1):inds_glob(3,2));
    
    Jout1_y = Jout1_y(1:L,1:M+1,1:N);
    for ii=1:num_diel
        JOut_full(inds_glob(num_diel+2+ii,1)+locs_diel_pos{ii}{2}-1) = Jout1_y(ids_panel_diel{ii}{2});
        JOut_full(inds_glob(num_diel+2+ii,1)+locs_diel_pos{ii}{5}-1) = -Jout1_y(ids_panel_diel{ii}{5});
        JOut_full(inds_glob(num_diel+2+ii,1):inds_glob(num_diel+2+ii,2))=coe_yy{ii}+JOut_full(inds_glob(num_diel+2+ii,1):inds_glob(num_diel+2+ii,2));
    end
    
    %     JOut_full(inds_glob(6,1)+locs_diel_pos{2}{2}-1) = Jout1_y(ids_panel_diel{2}{2});
    %     JOut_full(inds_glob(6,1)+locs_diel_pos{2}{5}-1) = -Jout1_y(ids_panel_diel{2}{5});
    %     JOut_full(inds_glob(6,1):inds_glob(6,2))=coe_yy2+JOut_full(inds_glob(6,1):inds_glob(6,2));
   
    Jout1_z = Jout1_z(1:L,1:M,1:N+1);
    for ii=1:num_diel
        JOut_full(inds_glob(2*(num_diel+1)+1+ii,1)+locs_diel_pos{ii}{3}-1) = Jout1_z(ids_panel_diel{ii}{3});
        JOut_full(inds_glob(2*(num_diel+1)+1+ii,1)+locs_diel_pos{ii}{6}-1) = -Jout1_z(ids_panel_diel{ii}{6});
        JOut_full(inds_glob(2*(num_diel+1)+1+ii,1):inds_glob(2*(num_diel+1)+1+ii,2))=coe_zz{ii}+JOut_full(inds_glob(2*(num_diel+1)+1+ii,1):inds_glob(2*(num_diel+1)+1+ii,2));
    end
%     
%     JOut_full(inds_glob(9,1)+locs_diel_pos{2}{3}-1) = Jout1_z(ids_panel_diel{2}{3});
%     JOut_full(inds_glob(9,1)+locs_diel_pos{2}{6}-1) = -Jout1_z(ids_panel_diel{2}{6});
%     JOut_full(inds_glob(9,1):inds_glob(9,2))=coe_zz2+JOut_full(inds_glob(9,1):inds_glob(9,2));
end
