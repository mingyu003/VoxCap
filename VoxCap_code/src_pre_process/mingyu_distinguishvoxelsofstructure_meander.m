function [idx,grid_intcon] = mingyu_distinguishvoxelsofstructure_meander(dx,r,Cnt,Dims,Orients,num_cond_x,num_cond_z)
...
tola=1e-13;
%org_comp_domain=[0,0,0];%r(1,1,1,:);



parfor kk=1:num_indv_elem
%     counter=0;
...
%     idx{kk}=idx{kk}(1:counter);
    idx{kk}=zeros(sum(boolean_tens),1);
...
parfor ii=1:num_cond_z
    for jj=1:2*num_cond_x-1
